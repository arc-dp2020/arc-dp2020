function sta = trainClassiferV2GA(matFold,trainSegFeat,trainSegGT,numClass,selClf,iCV)
...
for i = 1 : numClass
    ...
    if exist(([matFold 'SelFeatIndex_CV' num2str(iCV) '_' num2str(i) '.mat']),'file')
        ...
        ho = 0.1; 
        % Hold-out method
        HO = cvpartition(binSegGT,'HoldOut',ho,'Stratify',false);
        ...
        MR       = 0.05; 
        % Genetic Algorithm
        [sFeat,Sf,Nf,curve] = jGA2(trainSegFeat,binSegGT,N,max_Iter,CR,MR,HO);
        save([matFold 'SelFeatIndex_CV' num2str(iCV) '_' num2str(i) '.mat'],'Sf','curve');
    end
    
    selSegFeat = trainSegFeat(:, Sf);
