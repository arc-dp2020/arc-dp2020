...
for iImg = 1:lenImg
    if ismember(iImg,testImgIdx)  % testing images;
        ...
        if 1
            ...
            [gtHeight, gtWidth] = size(imgGT);
            %TT = imgGT;
            %TT = reshape(preLabel,[gtHeight, gtWidth]); % layer1 result
            TT = reshape(preLabel1,[gtHeight, gtWidth]); % 
                    ...
                    for iW = 1 : gtWidth
                        switch TT(iH,iW)
                            case 1
                                outGT(iH,iW,:) = [128 128 128];
                            case 2
                                outGT(iH,iW,:) = [0 128 64];
                            case 3
                                outGT(iH,iW,:) = [192 192 128];
                            case 4
                                outGT(iH,iW,:) = [128 64 128];
                            case 5
                                outGT(iH,iW,:) = [0 0 192];
                            ...
                            case 7
                                outGT(iH,iW,:) = [192 128 128];
                            case 8
                                outGT(iH,iW,:) = [64 64 128];
                            case 9
                                outGT(iH,iW,:) =  [64 0 128];
                            case 10
                                outGT(iH,iW,:) =  [64 64 0];
                            case 11
                                outGT(iH,iW,:) =  [0 128 192];
                            otherwise
                                outGT(iH,iW,:) = [0 0 0];
                        end
                    end
...
save([matFold '_ClsOutPutCV' num2str(iCV) 'Block' num2str(blockrow) '.mat'],'allGT','allPL','allPL1','allPL2','allPL3');
